...
namespace JetBrains.ForTea.RiderPlugin.TemplateProcessing.CodeGeneration.Reference.Impl
{
	...
	public sealed class T4ReferenceExtractionManager : IT4ReferenceExtractionManager
	{
		...
		{
			directDependencies.Add(ResolveReference(file, "mscorlib"));
			directDependencies.Add(ResolveReference(file, "System"));
			directDependencies.AddRange(
				Environment.TextTemplatingAssemblyNames.Select(assemblyName => ResolveReference(file, assemblyName))
			);
		}
